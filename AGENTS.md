...
**不要自动操作模拟器或真机。** 需要设备侧验证时，Agent 只负责：

1. 编译并安装好应用（build / install 可以做）；
2. 给出清晰的手动操作流程（点哪个页面、输入什么、点什么按钮）；
3. 给出需要抓取的日志命令或日志过滤关键字（如 `hilog | grep xxx`、关注的 Tag）；
4. 由用户手动操作设备并回贴日志或现象，Agent 根据反馈继续分析。

禁止使用截屏 + 图片识别的方式做 UI 自动化（如 uitest 点击坐标、snapshot_display 截图回看），这类方式耗费大量 token 且不可靠。例外：用户明确要求 Agent 自动操作设备时除外。
...
项目中的跨站经验提示词统一维护在 `entry/src/main/ets/engine/ai/AiPromptKnowledge.ts`。
经验提示词不是模型猜测的备忘录，只有完成问题定位并验证有效后才能加入经验库：

1. **必须有可复现依据**：记录对应阶段、失败现象、触发条件和实际页面/接口结构。仅凭一次模型输出或主观推测，不得新增提示词。

2. **必须真实验证**：新增提示词后，至少使用一个真实 HTML/JSON 页面，通过 `SourceExecutor` 执行生成的规则并验证目标字段；涉及搜索、详情、目录、正文的提示词，还应尽可能跑通该阶段的后续链路。不能只验证模型是否“理解了提示词”。

3. **优先验证通用语义**：提示词应描述可跨站复用的 Legado 规则语义（例如卡片内字段、语义标签、显式 `@href`/`@src`、正文 `@textNodes`），不得把某个样本书的数字 ID、固定 URL、Cookie、Token 或验证码写入经验库。

4. **区分代码缺陷和源规则缺陷**：如果验证证明是执行器没有实现 Android Legado 语义，先修执行器；只有确认模型识别策略确实需要补充时，才新增提示词。

5. **保留验证记录**：提交经验提示词时，在提交说明或相邻注释中标明验证站点/页面类型、验证结果和适用阶段；日志中应能看到对应的经验 ID。未通过验证的内容只能留在问题记录或临时文件中，不能进入正式经验库。

6. **控制提示词规模**：每条经验只解决一个可识别问题，避免复制整段历史日志；经验检索应保持有限条目，并以当前页面真实 DOM/JSON 为最终依据。
